...
class my_model():
    def __init__(self, adata,args, random_seed=50, add_regularization=True, device='cpu'):
    ...
    def train_model(self):
       if self.add_regularization:
          ...
          #preprocess(adata)
          get_feature(adata)
          model = Train(adata,self.args, device=self.device)
          emb = model.train()
          ...
          #preprocess(adata)
          get_feature(adata)
          model = Train(adata,self.args, add_regularization=True, device=self.device)
          emb_regularization = model.train()
          self.adata_output.obsm['emb_reg'] = emb_regularization
          
       else:
          model = Train(self.adata.copy(),self.args)
          emb= model.train()
          self.adata_output.obsm['emb'] = emb
...
class Train():
    def __init__(self,
       ...
       # self.label_CSL = torch.FloatTensor(self.label_CSL).to(self.device)
        ...
        Val=gu.augmentation(self.adata,self.edge_index,aug_type=self.args.augment_list)
        if self.args.aug_type==0:
           self.features_a=Val
           self.features_a = torch.FloatTensor(self.features_a).to(self.device)
           self.adj=self.adata.obsm['adj']
        elif self.args.aug_type==1:
             self.features_a =self.features
             self.adj = adata.obsm['adj_new']
        else:
            self.features_a=Val
            self.features_a = torch.FloatTensor(self.features_a).to(self.device)
            self.adj = adata.obsm['adj_new']
